...
function all_in_one_gallery_options() {
	...
	foreach($category as $cat){
		...
		//echo '<li'.$sel_style.'><input type="checkbox"'.$sel.' name="'."allInOneGallery_ecluded_cat[{$cat->cat_name}]".'"/><label>'.$cat->cat_name.'</label><a title="Exclude images from: <strong>'.$cat->cat_name.'</strong>" href="'.allInOneGalleryURLDirect().'/select.php?catID='.$cat->cat_ID.'"'.$thickBox.'>'.$hasImages.'</a></li>';
	...
	//echo '<li'.$sel_style.'><input type="checkbox"'.$sel.' name="'."allInOneGallery_ecluded_cat[Pages]".'"/><label>Pages</label><a title="Exclude images from the <strong>Pages</strong>" href="'.allInOneGalleryURLDirect().'/select.php?catID=Pages"'.$thickBox.'>'.$hasImages.'</a></li>';
			...
			//No choice means just arrived on the gallery
				...
				foreach($myposts as $post){
					...
					$post_attachments = get_children('post_type=attachment&post_mime_type=image&post_parent='.$postID);
						...
						//allInOneGalleryDebug('[gallery id="'.$postID.'"'.$userColumnsChoice.$myImgSize.$excludedList.']');
						
						//Build gallery
						$content .= '<div class="a_gallerie">';
						$content .= '[gallery id="'.$postID.'"'.$userColumnsChoice.$myImgSize.$excludedList.']';
						$content .= '</div>';
					}
				}
	                	//If there is no posts with attachements, say it
